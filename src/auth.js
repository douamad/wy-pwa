 ...
 */
const REFRESH_TOKEN_URL = 'http://localhost:8000/api/login'
...
const AUTH_BASIC_HEADERS = {
  headers: {
    'Authorization': 'Basic MTpMa2dia1hDTXFqdFFYVlJKNEVjNjU2VURvWlYwZEU3WTUwU3lZY1ZS' // Base64(client_id:client_secret) "demoapp:demopass"
  },
  emulateJSON: true
}
...
export default {
  ...
  login (creds, redirect) {
    const params = { 'grant_type': 'password', 'email': creds.email, 'password': creds.password }
  ...
  _storeToken (response) {
    ...
    auth.refreshToken = response.body.access_token
    // TODO: get user's name from response from Oauth server.
    user.firstName = response.body.user.first_name
